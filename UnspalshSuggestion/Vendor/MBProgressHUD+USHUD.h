...
/// 主线程安全 ，确保在主队列执行
#ifndef USdispatch_queue_async_safe
#define USdispatch_queue_async_safe(queue, block)\
if (strcmp(dispatch_queue_get_label(DISPAUSH_CURRENT_QUEUE_LABEL), dispatch_queue_get_label(queue)) == 0) {\
block();\
} else {\
...
#ifndef USdispatch_main_async_safe
#define USdispatch_main_async_safe(block) USdispatch_queue_async_safe(dispatch_get_main_queue(), block)
#endif
